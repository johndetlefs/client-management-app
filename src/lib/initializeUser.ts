 ...
 */
export async function initializeNewUser(userId: string, email: string): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        await adminDb.collection('tenants').doc(tenantId).collection('users').doc(userId).set({
            uid: userId,
            email,
            role: 'owner',
